...
def main() -> None:
    print("Hello from nldate!")from __future__ import annotations
...
def _add_years(value: date, years: int) -> date:
    try:
        ...
        # Clamp leap day to Feb 28 in non-leap years.
        return value.replace(month=2, day=28, year=value.year + years)
...
def parse(s: str, today: date | None = None) -> date:
    ...
    if match := re.fullmatch(r"(\d+) (day|days|year|years) (before|after) (.+)", text):
        amount_str, unit, relation, target_text = match.groups()
        amount = int(amount_str)
        target = parse(target_text, today=today)
